...
def to_number(value, base):
    value = str(value)
    base = int(base)
    d = base if digit_strings.get(base) else None
    ...
    for i, char in enumerate(value):
        if digit_strings[d].find(char) != -1 and digit_strings[d].find(char) < base:
            number += digit_strings[d].find(char) * base**(decimal - i)
    return number
...
def to_base(number, base, precision=0):
    ...
    try:
        for i in reversed(xrange(precision + 1 + int_log(number, base))):
            ...
            value += digit_strings[d][x]
    except ValueError:
        value = digit_strings[d][0]
    if precision:
